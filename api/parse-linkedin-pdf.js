// LinkedIn PDF Parser - Vercel Serverless Function
...
// Parser logic extracted from server/pdfParser.js
function parseLinkedInPDF(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Generic name detection (first text line that looks like a name)
        if (!resume.header.name && type === 'TEXT' && i < 5 && /^[A-ZÄÖÜ][a-zäöü]+\s+[A-ZÄÖÜ][a-zäöü]+/.test(text)) {
            resume.header.name = text;
            let j = i + 1;
            let titleParts = [];
            while (j < lines.length && lines[j].type !== 'HEADER') {
